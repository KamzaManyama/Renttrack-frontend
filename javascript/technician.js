...
if(typeof Auth!=='undefined'){
  ...
  if(!['technician','staff'].includes(user?.role)){window.location.href='index.html';}
...
function showPage(id,navEl){
  ...
  const t={jobs:'My Jobs',completed:'Completed Jobs',notifications:'Notifications'};
  document.getElementById('page-title').textContent=t[id]||'';
  if(id==='jobs')loadJobs();
  if(id==='completed')loadCompleted();
  if(id==='notifications')loadNotifications();
}
function logout(){if(typeof Auth!=='undefined')Auth.clear();sessionStorage.setItem('rt_logged_out','1');window.location.replace('index.html');}
...
function filterJobs(el){document.querySelectorAll('.filter-pill').forEach(p=>p.classList.remove('active'));el.classList.add('active');currentJobFilter=el.dataset.status||'';jobsPage=1;applyJobFilters();}
...
function renderJobsGrid(f){
  ...
  grid.innerHTML=page.map(t=>{
    ...
    return `<div class="job-card priority-${t.priority||'medium'}" onclick="openJobModal('${t.id}')">
      <div class="job-card-title">${t.title||'Untitled'}</div>
      <div class="job-card-number">${t.ticket_number||'—'}</div>
      <div class="job-card-badges">
        ${mb(sl,SC[t.status]||'badge-closed',SI[t.status]||'')}
        ${mb(pl,PC[t.priority]||'badge-medium',PI[t.priority]||'')}
        ${t.category_name?`<span class="badge badge-category"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>${t.category_name}</span>`:''}
      </div>
      <div class="job-card-meta">
        <span class="job-card-meta-item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>${t.property_name||'—'}</span>
        ${t.unit_number?`<span class="job-card-meta-item"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>Unit ${t.unit_number}</span>`:''}
...
function changeJobsPage(p){
  const s=(document.getElementById('job-search')?.value||'').toLowerCase();
  const prio={urgent:0,high:1,medium:2,low:3};
  let f=activeJobs.filter(t=>{const ms=!s||t.title.toLowerCase().includes(s)||(t.ticket_number||'').toLowerCase().includes(s);const mf=!currentJobFilter||t.status===currentJobFilter;return ms&&mf;});
  f.sort((a,b)=>(prio[a.priority]??4)-(prio[b.priority]??4)||new Date(b.created_at)-new Date(a.created_at));
...
async function quickStatus(id,status){
  try{await POST(`/api/tickets/${id}/status`,{status});const m={completed:'Job completed',in_progress:'Job started',on_hold:'Job put on hold'};toast(m[status]||'Status updated');loadJobs();}catch(e){toast(e.message,'error');}
}
...
async function openJobModal(id){
  ...
  try{
    const [tkR,updR]=await Promise.all([GET(`/api/tickets/${id}`),GET(`/api/tickets/${id}/updates`)]);
    const t=tkR.data,updates=updR.data||[];
    document.getElementById('modal-tk-num').textContent=t.ticket_number||'—';
    document.getElementById('modal-tk-badges').innerHTML=`${mb((t.status||'unknown').replace(/_/g,' '),SC[t.status]||'badge-closed',SI[t.status]||'')} ${mb(t.priority||'unknown',PC[t.priority]||'badge-medium',PI[t.priority]||'')} ${t.category_name?`<span class="badge badge-category"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>${t.category_name}</span>`:''}`;
    ...
    document.getElementById('modal-tk-body').innerHTML=`
      ...
      <div class="detail-grid">
        <div class="detail-block">
          ...
          ${t.unit_number?`<div class="detail-block-sub"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>Unit ${t.unit_number}</div>`:''}
          ${t.area_name?`<div class="detail-block-sub">${t.area_name}</div>`:''}
        </div>
        <div class="detail-block">
          ...
          <div class="detail-block-sub"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Reported by <strong style="color:var(--text);margin-left:3px">${t.created_by_name||'—'}</strong></div>
...
async function quickUpdateStatus(){
  ...
  try{await POST(`/api/tickets/${openJobId}/status`,{status});const m={completed:'Job marked complete',in_progress:'Job started',on_hold:'Job put on hold'};toast(m[status]||'Status updated');closeJobModal();loadJobs();}catch(e){toast(e.message,'error');}
}

async function postComment(){
  const msg=document.getElementById('modal-comment').value.trim();if(!msg)return;
  try{await POST(`/api/tickets/${openJobId}/updates`,{message:msg,is_internal:false});document.getElementById('modal-comment').value='';toast('Update posted');openJobModal(openJobId);}catch(e){toast(e.message,'error');}
}
...
async function loadCompleted(){
  ...
  try{const resp=await GET('/api/tickets?status=completed');allCompleted=resp.data||[];filteredCompleted=[...allCompleted];completedPage=1;renderCompletedTable();}catch(e){toast(e.message,'error');}
}
...
function renderCompletedTable(){
  ...
  if(!filteredCompleted.length){tbody.innerHTML=`<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--muted);font-size:13px;">No completed jobs found.</td></tr>`;document.getElementById('completed-pagination').innerHTML='';return;}
  const page=filteredCompleted.slice((completedPage-1)*CPP,completedPage*CPP);
  tbody.innerHTML=page.map(t=>`
    <tr onclick="openJobModal('${t.id}')">
      <td data-label="Ticket #"><span style="font-family:'JetBrains Mono',monospace;font-size:11.5px;color:var(--muted);font-weight:500;">${t.ticket_number||'—'}</span></td>
      <td data-label="Title"><span style="font-weight:500;text-transform:capitalize;color:var(--text);">${t.title||'—'}</span></td>
      <td data-label="Property / Unit">
        <div style="font-size:13px;color:var(--text-2);font-weight:500;">${t.property_name||'—'}</div>
        ${t.unit_number?`<div style="font-size:11.5px;color:var(--muted);margin-top:2px;display:flex;align-items:center;gap:3px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>Unit ${t.unit_number}</div>`:''}
      </td>
      <td data-label="Category"><span style="font-size:12.5px;color:var(--text-2);">${t.category_name||'—'}</span></td>
...
function changeCompletedPage(p){const max=Math.ceil(filteredCompleted.length/CPP);if(p<1||p>max)return;completedPage=p;renderCompletedTable();window.scrollTo({top:0,behavior:'smooth'});}
...
async function loadNotifications(){
  ...
  try{
    ...
    list.innerHTML=notifs.length?notifs.map(n=>{if(!n.is_read)unread++;return `<div style="display:flex;align-items:flex-start;gap:14px;padding:16px 20px;border-bottom:1px solid var(--border-light);cursor:pointer;transition:background .15s;${!n.is_read?'background:#f0f9ff;':''}" id="notif-${n.id}" onclick="readNotif('${n.id}')"><div style="width:8px;height:8px;border-radius:50%;margin-top:6px;flex-shrink:0;background:${n.is_read?'transparent':'var(--tech)'}"></div><div style="flex:1;min-width:0;"><div style="font-weight:${n.is_read?500:700};font-size:13.5px;margin-bottom:3px;color:var(--text)">${n.title||'Notification'}</div><div style="font-size:13px;color:var(--text-2);line-height:1.5;">${n.message||''}</div><div style="font-size:11.5px;color:var(--muted);margin-top:6px;display:flex;align-items:center;gap:4px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>${typeof timeAgo==='function'?timeAgo(n.created_at):n.created_at}</div></div></div>`;}).join(''):'<div style="text-align:center;padding:48px;color:var(--muted);font-size:13px;">No notifications yet.</div>';
    document.getElementById('notif-dot').style.display=unread>0?'inline-block':'none';
  }catch(e){toast(e.message,'error');}
}
async function readNotif(id){try{await PUT(`/api/notifications/${id}/read`,{});const el=document.getElementById(`notif-${id}`);if(el){el.style.background='transparent';const d=el.querySelector('div[style*="border-radius:50%"]');if(d)d.style.background='transparent';}}catch(e){}}
async function markAllRead(){try{await PUT('/api/notifications/read-all',{});document.getElementById('notif-dot').style.display='none';toast('All notifications marked as read');loadNotifications();}catch(e){toast(e.message,'error');}}
